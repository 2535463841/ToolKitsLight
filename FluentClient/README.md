...
批量执行命令
```
root@LAPTOP-BFO405GC:~# fluent-utils ssh-cmd hosts.txt pwd
...
批量拷贝文件到远程主机的 `/tmp` 目录
```
root@LAPTOP-BFO405GC:~# fluent-utils scp-put hello.jpg hosts.txt --remote /tmp/
...
```
root@LAPTOP-BFO405GC:~# fluent-utils scp-get hosts.txt --remote /tmp/hello.jpg  --local remote_files
...
```
root@LAPTOP-BFO405GC:~# fluent-utils qrcode-parse 'hello word!'
█▀▀▀▀▀█ ▀█▄▄▀ █▀▀▀▀▀█
█ ███ █ ▀█ ▄█ █ ███ █
█ ▀▀▀ █ ▄█▄██ █ ▀▀▀ █
▀▀▀▀▀▀▀ ▀ ▀ █ ▀▀▀▀▀▀▀
...
```
root@LAPTOP-BFO405GC:~# fluent-utils qrcode-dump hello.jpg 
hello word!
```
